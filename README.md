...
脚本启动后**最先执行**环境检测（先检测、清理，再开始调优），避免多次调优后文件堆积、参数互相覆盖：
...
3. **冲突提醒**：存在多个 `zz*` 前缀 sysctl 文件时列出生效顺序（字典序最后的生效）
4. **调优前清理**：列出 AI/安装器生成的残留文件并给出可复制的 `rm -f` 命令，询问 `[y/N]`，`y` 直接删除（系统默认/其他来源文件不会进删除清单）
5. **本次写入地址**：应用前明确列出本次将写入/更新的全部文件路径

每次应用后，文件地址会追加记录到 `/root/tcp-tune-file-log.txt`，方便追溯每次调优动了哪些文件。本工具自身管理的 `zzz-tcp-tune.conf` 等文件无需手动删除，每次运行会覆盖。
